...
pub const TokenBoundary = struct {
    ...
    /// Dispatch six 64-row Q4_0 DMMV ranges that share one input vector in one CS submission.
    ///
    /// This extends the routed MoE gate/up fast path to cover the always-active
    /// shared expert gate/up prefix in the same fence as two routed experts.
    /// The output slice receives A, B, C, D, E, F ranges in order.
    pub fn dmmvQ4_0SixRowRangesParallel64(
        self: *TokenBoundary,
        input: []const f32,
        weights_a_q4_0: []const u8,
        weights_b_q4_0: []const u8,
        weights_c_q4_0: []const u8,
        weights_d_q4_0: []const u8,
        weights_e_q4_0: []const u8,
        weights_f_q4_0: []const u8,
        cols: u32,
        output: []f32,
    ) !void {
        const rows_per_range: u32 = 64;
        const rows: u32 = rows_per_range * 6;
        if (cols == 0 or cols % 32 != 0) return error.ShapeMismatch;
        if (input.len < cols or output.len < rows) return error.ShapeMismatch;

        const row_bytes: usize = (@as(usize, cols) / 32) * 18;
        const range_bytes = @as(usize, rows_per_range) * row_bytes;
        const weights_bytes = range_bytes * 6;
        if (weights_a_q4_0.len < range_bytes or
            weights_b_q4_0.len < range_bytes or
            weights_c_q4_0.len < range_bytes or
            weights_d_q4_0.len < range_bytes or
            weights_e_q4_0.len < range_bytes or
            weights_f_q4_0.len < range_bytes)
        {
            return error.ShapeMismatch;
        }
        ...
        @memcpy(self.input_map[weight_off + range_bytes * 2 ..][0..range_bytes], weights_c_q4_0[0..range_bytes]);
        @memcpy(self.input_map[weight_off + range_bytes * 3 ..][0..range_bytes], weights_d_q4_0[0..range_bytes]);
        @memcpy(self.input_map[weight_off + range_bytes * 4 ..][0..range_bytes], weights_e_q4_0[0..range_bytes]);
        @memcpy(self.input_map[weight_off + range_bytes * 5 ..][0..range_bytes], weights_f_q4_0[0..range_bytes]);
        ...
        const signal_expected: u64 = 0x5A494E435254_B500 | @as(u64, self.submit_count + 1);
        self.builder.reset();
        try self.builder.writeNop(1);

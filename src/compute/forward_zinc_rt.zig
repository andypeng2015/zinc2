...
const ScalarDecodeState = struct {
    ...
    direct_ssm_alpha_q8_row_range_slice_successes: u32 = 0,
...
fn directLmHeadDecodeCadenceForEnv(raw_override: ?[]const u8) u32 {
    const raw = raw_override orelse return direct_lm_head_decode_cadence_default;
    if (std.fmt.parseInt(u32, raw, 10) catch null) |parsed| {
        return parsed;
    }
    return direct_lm_head_decode_cadence_default;
}
...
fn canTrustDirectSsmRowRangeWithoutOracle(state: *const ScalarDecodeState, tensor_type: gguf.GGMLType) bool {
    return switch (tensor_type) {
        .f32, .q8_0 => canTrustDirectSsmQ8_0RowRangeWithoutOracle(state),
        else => false,
    };
}
...
fn scalarEvalTokenDense(
    ...
    if (direct_compute_tracking) |tracking| {
        if (tracking.phase == .decode and !tracking.lm_head_only) state.beginDirectDecodeSlice();
    }
    const layer_direct_tracking: ?DirectComputeTracking = if (direct_compute_tracking) |tracking|
        if (tracking.lm_head_only) null else tracking
    else
        null;
    ...
    for (model.layer_tensors, 0..) |lt, li| {
        ...
        try runAttentionLayer(model, state, lt, layer, position, layer_direct_tracking);
...
const DirectLmHeadWindow = struct {
    start: u32 = 0,
    rows: u32 = 0,
};
...
fn consumeDirectLmHeadQ4_0ArgmaxPrefix(
    ...
    const tracking = maybe_tracking orelse return null;
    const cols: u32 = @intCast(state.norm.len);
    if (cols == 0 or cols % 32 != 0) return null;
    ...
    const gpu_best_delta = @abs(cpu_gpu_best - selection.best.value);
    if (gpu_best_delta > direct_lm_head_q4_0_best_row_tolerance) {
        log.warn("M1 AMDGPU CS direct LM-head Q4_0 argmax-prefix mismatch: row={d} cpu={d:.6} gpu={d:.6} abs_delta={d:.6}; selected token remains host-computed", .{
            selection.best.index,
            cpu_gpu_best,
            selection.best.value,
            gpu_best_delta,
...
fn directLmHeadQ4_0GpuRow(
    ...
    cols: u32,
    row_bytes: usize,
    candidate: ScoredToken,
    label: []const u8,
) ?DirectLmHeadGpuRow {
    if (candidate.index >= lm_head_rows) return null;
    if (!std.math.isFinite(candidate.value)) return null;
    if (state.row_scratch.len < 1) return null;

    const row_off = @as(usize, candidate.index) * row_bytes;
    if (row_off + row_bytes > q4_0_raw.len) return null;
    ...
    tracking.boundary.dmmvQ4_0RowRange(
        ...
        log.warn("M1 AMDGPU CS direct LM-head Q4_0 top2-{s} row unavailable ({s}); selected token remains host-computed", .{ label, @errorName(err) });
        return null;
    };

    const gpu_value = gpu_logits[0];
    if (!std.math.isFinite(gpu_value)) {
        log.warn("M1 AMDGPU CS direct LM-head Q4_0 top2-{s} row produced non-finite row {d}; selected token remains host-computed", .{ label, candidate.index });
        return null;
    }
    const delta = @abs(gpu_value - candidate.value);
    if (delta > direct_lm_head_q4_0_best_row_tolerance) {
        log.warn("M1 AMDGPU CS direct LM-head Q4_0 top2-{s} row mismatch: row={d} cpu={d:.6} gpu={d:.6} abs_delta={d:.6}; selected token remains host-computed", .{
            label,
            candidate.index,
            candidate.value,
            gpu_value,
    ...
    return .{ .value = gpu_value, .delta = delta };
}

fn consumeDirectLmHeadQ4_0Top2Rows(
    state: *ScalarDecodeState,
    maybe_tracking: ?DirectComputeTracking,
    q4_0_raw: []const u8,
    lm_head_rows: u32,
    cpu_selection: ArgmaxTop2Result,
) ?ArgmaxTop2Result {
    const tracking = maybe_tracking orelse return null;
    const cols: u32 = @intCast(state.norm.len);
    if (cols == 0 or cols % 32 != 0) return null;
    if (cpu_selection.best.index >= lm_head_rows or !std.math.isFinite(cpu_selection.best.value)) return null;
    ...
    var gpu_selection: ArgmaxTop2Result = .{};
    gpu_selection.offer(cpu_selection.best.index, gpu_best.value);
    var rows_consumed: u32 = 1;
    var second_delta: f32 = 0.0;
    var checked_second = false;

    if (cpu_selection.second.index < lm_head_rows and std.math.isFinite(cpu_selection.second.value)) {
        const gpu_second = directLmHeadQ4_0GpuRow(
            state,
            tracking,
            q4_0_raw,
            lm_head_rows,
            cols,
            row_bytes,
            cpu_selection.second,
            "second",
        ) orelse return null;
        gpu_selection.offer(cpu_selection.second.index, gpu_second.value);
        rows_consumed += 1;
        second_delta = gpu_second.delta;
        checked_second = true;
    } else {
        gpu_selection.second = cpu_selection.second;
    }

    tracking.ops.* += rows_consumed;
    mergeDirectComputeKind(tracking.kind, .dmmv_row_range);
    tracking.consumed.* = true;
    tracking.real_model_slice.* = true;
    if (tracking.phase == .decode) {
        if (tracking.decode_model_slices) |slices| slices.* += rows_consumed;
    }
    if (tracking.selected_token) |token| token.* = gpu_selection.best.index;
    log.info("M1 AMDGPU CS direct model slice consumed: direct_compute_ops={d} direct_compute_kind=dmmv_row_range op=lm_head_q4_0_top2_rows phase={s} rows={d} cols={d} token={d} gpu={d:.6} second=({d},{d:.6}) abs_delta_best={d:.6} abs_delta_second={d:.6} selected_source=gpu_top2_rows consumed_gpu_model_value=1", .{
        tracking.ops.*,
        directComputePhaseName(tracking.phase),
        rows_consumed,
        cols,
...
fn scalarEvalToken(
    ...
    const begin_model_direct_decode_slice = if (direct_compute_tracking) |tracking|
        tracking.phase == .decode and !tracking.lm_head_only
    else
        false;
    const begin_router_direct_decode_slice = if (direct_router_tracking) |tracking|
        tracking.phase == .decode
    else
        false;
    const begin_full_direct_decode_slice = begin_model_direct_decode_slice or begin_router_direct_decode_slice;
    if (begin_full_direct_decode_slice) state.beginDirectDecodeSlice();
    const layer_direct_tracking: ?DirectComputeTracking = if (direct_compute_tracking) |tracking|
        if (tracking.lm_head_only) null else tracking
    else
        null;
    ...
    for (model.layer_tensors, 0..) |lt, li| {
        ...
        if (is_ssm_layer) {
            try runSsmLayer(model, state, lt, layer, layer_direct_tracking);
        } else {
            try runAttentionLayer(model, state, lt, layer, position, layer_direct_tracking);
        }
        ...
        if (lt.ffn_gate_up_exps != null and cfg.is_gemma) {
            ...
            try runMoeLayer(model, state, lt, layer_direct_tracking, direct_router_tracking);
        } else {
            try runDenseFfnLayer(model, state, lt);
        }
...
fn runSsmLayer(
    ...
    if (direct_alpha or direct_beta) {
        ...
        if (direct_alpha and direct_beta and consumeDirectSsmAlphaBetaRowRange(model, state, lt, layer, direct_compute_tracking)) {
            consumed_alpha = true;
            consumed_beta = true;
        }
...
fn directSsmRowBytes(tensor_type: gguf.GGMLType, cols: u32) usize {
    return switch (tensor_type) {
...
fn directSsmSupportsRowRange(tensor_type: gguf.GGMLType, cols: u32) bool {
    return switch (tensor_type) {
        .f32 => cols != 0 and cols % 64 == 0,
        .q8_0 => cols != 0 and cols % 32 == 0,
        else => false,
    };
}

fn directSsmDispatchRowRange(
    boundary: *zinc_rt.cs.TokenBoundary,
    ...
    return switch (tensor_type) {
        .f32 => boundary.dmmvF32RowRange(input, weights, rows, cols, output),
        .q8_0 => boundary.dmmvQ8_0RowRange(input, weights, rows, cols, output),
        else => error.UnsupportedType,
    };
}

fn directSsmDispatchTwoRowRanges(
    boundary: *zinc_rt.cs.TokenBoundary,
    tensor_type: gguf.GGMLType,
    input: []const f32,
    weights_a: []const u8,
    ...
    switch (tensor_type) {
        .f32 => {
            try boundary.dmmvF32TwoRowRanges(input, weights_a, rows_a, weights_b, rows_b, cols, output);
            return false;
        },
        .q8_0 => {
            const use_parallel64 = rows_a + rows_b == 64;
            if (use_parallel64) {
                try boundary.dmmvQ8_0TwoRowRangesParallel64(input, weights_a, rows_a, weights_b, rows_b, cols, output);
            } else {
                try boundary.dmmvQ8_0TwoRowRanges(input, weights_a, rows_a, weights_b, rows_b, cols, output);
            }
            return use_parallel64;
        },
        else => return error.UnsupportedType,
    }
}

fn directSsmAlphaQ8_0RowRangeRows(
    cfg: CpuModelConfig,
    state: *const ScalarDecodeState,
    alpha_info: gguf.TensorInfo,
) u32 {
    const cols: u32 = @intCast(state.norm.len);
    if (!directSsmSupportsRowRange(alpha_info.type_, cols)) return 0;
    if (cfg.ssm_dt_rank == 0 or state.alpha.len < cfg.ssm_dt_rank) return 0;
    if (state.row_scratch.len < @as(usize, cfg.ssm_dt_rank) * 2) return 0;
    return cfg.ssm_dt_rank;
}

fn canAttemptDirectSsmAlphaQ8_0RowRange(
    model: *const Model,
    state: *const ScalarDecodeState,
    lt: LayerTensors,
    layer: u32,
    ...
    if (state.direct_ssm_alpha_q8_row_range_slice_successes >= state.direct_ssm_q8_row_range_max_successes) return false;
    ...
    const row_bytes = directSsmRowBytes(alpha_info.type_, @intCast(state.norm.len));
    if (row_bytes == 0) return false;
    const alpha_raw = model.tensorData(alpha_info);
    return alpha_raw.len >= @as(usize, rows) * row_bytes;
}

fn consumeDirectSsmAlphaQ8_0RowRange(
    model: *const Model,
    state: *ScalarDecodeState,
    lt: LayerTensors,
    layer: u32,
    ...
    if (rows == 0) return false;
    const cols: u32 = @intCast(state.norm.len);
    const tensor_type = alpha_info.type_;
    const type_name = directSsmRowRangeTypeName(tensor_type);

    const alpha_raw = model.tensorData(alpha_info);
    const row_bytes = directSsmRowBytes(tensor_type, cols);
    if (row_bytes == 0) return false;
    const rows_usize: usize = @intCast(rows);
    const weight_bytes = rows_usize * row_bytes;
    if (alpha_raw.len < weight_bytes) return false;

    const trusted_no_oracle = canTrustDirectSsmRowRangeWithoutOracle(state, tensor_type);
    const required_scratch_rows = if (trusted_no_oracle) rows_usize else rows_usize * 2;
    ...
    if (!trusted_no_oracle) {
        ...
        matvecRawDirectSerial(alpha_raw, tensor_type, state.norm, null, 0, rows, cpu_out) catch {
            return false;
        };
    }
    directSsmDispatchRowRange(
        tracking.boundary,
        tensor_type,
        state.norm,
        alpha_raw[0..weight_bytes],
        rows,
        cols,
        gpu_out,
    ) catch |err| {
        log.warn("M1 AMDGPU CS direct SSM alpha {s} row-range unavailable ({s}); alpha remains host-computed", .{ type_name, @errorName(err) });
        return false;
    };
    ...
    if (trusted_no_oracle) {
        for (gpu_out, 0..) |gpu_value, i| {
            if (!std.math.isFinite(gpu_value)) {
                log.warn("M1 AMDGPU CS direct trusted SSM alpha {s} row-range produced non-finite row {d}; alpha remains host-computed", .{ type_name, i });
        ...
        for (gpu_out, 0..) |gpu_value, i| {
            if (!std.math.isFinite(gpu_value)) {
                log.warn("M1 AMDGPU CS direct SSM alpha {s} row-range produced non-finite row {d}; alpha remains host-computed", .{ type_name, i });
                markDirectSsmQ8_0RowRangeFailed(state, layer, .alpha);
                return false;
            }
        ...
        if (max_abs_delta > direct_ssm_alpha_q8_0_row_range_tolerance) {
            log.warn("M1 AMDGPU CS direct SSM alpha {s} row-range mismatch: layer={d} rows={d} max_abs_delta={d:.6} row={d}; alpha remains host-computed", .{
                type_name,
                layer,
                rows,
                max_abs_delta,
    ...
    state.direct_ssm_alpha_q8_row_range_slice_successes += 1;
    tracking.ops.* += 1;
    mergeDirectComputeKind(tracking.kind, .dmmv_row_range);
    tracking.consumed.* = true;
    tracking.real_model_slice.* = true;
    if (tracking.decode_model_slices) |slices| slices.* += 1;
    if (trusted_no_oracle) {
        log.info("M1 AMDGPU CS direct model slice consumed: direct_compute_ops={d} direct_compute_kind=dmmv_row_range op=ssm_alpha_{s}_row_range_trusted phase=decode layer={d} rows={d} cols={d} trust_after_successes={d} validation=finite_only", .{
            tracking.ops.*,
            type_name,
            layer,
            rows,
        ...
        log.info("M1 AMDGPU CS direct model slice consumed: direct_compute_ops={d} direct_compute_kind=dmmv_row_range op=ssm_alpha_{s}_row_range phase=decode layer={d} rows={d} cols={d} max_abs_delta={d:.6} max_row={d}", .{
            tracking.ops.*,
            type_name,
            layer,
            rows,
...
fn directSsmAlphaBetaRowRangeOpName(tensor_type: gguf.GGMLType, used_parallel64: bool, trusted: bool) []const u8 {
    return switch (tensor_type) {
        .f32 => if (trusted) "ssm_alpha_beta_f32_row_range_trusted" else "ssm_alpha_beta_f32_row_range",
        .q8_0 => if (used_parallel64)
            if (trusted) "ssm_alpha_beta_q8_0_row_range_parallel64_trusted" else "ssm_alpha_beta_q8_0_row_range_parallel64"
        else if (trusted) "ssm_alpha_beta_q8_0_row_range_trusted" else "ssm_alpha_beta_q8_0_row_range",
        else => "ssm_alpha_beta_unsupported_row_range",
    };
}

fn consumeDirectSsmAlphaBetaRowRange(
    model: *const Model,
    state: *ScalarDecodeState,
    lt: LayerTensors,
    layer: u32,
    ...
    if (state.direct_ssm_q8_row_range_max_successes == 0) return false;
    if (state.direct_ssm_alpha_q8_row_range_slice_successes >= state.direct_ssm_q8_row_range_max_successes) return false;
    if (state.direct_ssm_beta_q8_row_range_slice_successes >= state.direct_ssm_q8_row_range_max_successes) return false;
    if (directSsmLayerMaskContains(state.direct_ssm_alpha_q8_row_range_done_mask, layer)) return false;
    ...
    if (alpha_info.type_ != beta_info.type_) return false;
    const tensor_type = alpha_info.type_;
    ...
    if (!trusted_no_oracle) {
        ...
        matvecRawDirectSerial(alpha_raw, tensor_type, state.norm, null, 0, alpha_rows, cpu_alpha) catch {
            return false;
        };
        matvecRawDirectSerial(beta_raw, tensor_type, state.norm, null, 0, beta_rows, cpu_beta) catch {
            return false;
        };
    }

    const used_parallel64 = directSsmDispatchTwoRowRanges(
        tracking.boundary,
        tensor_type,
        state.norm,
        ...
        log.warn("M1 AMDGPU CS direct SSM alpha/beta {s} row-range unavailable ({s}); alpha/beta remain host-computed", .{ type_name, @errorName(err) });
        return false;
    };
    ...
    if (trusted_no_oracle) {
        for (gpu_out[0..alpha_rows_usize], 0..) |gpu_value, i| {
            if (!std.math.isFinite(gpu_value)) {
                log.warn("M1 AMDGPU CS direct trusted SSM alpha/beta {s} row-range produced non-finite alpha row {d}; alpha remains host-computed and beta may retry individually", .{ type_name, i });
                markDirectSsmQ8_0RowRangeFailed(state, layer, .alpha);
                return false;
            }
        }
        for (gpu_out[alpha_rows_usize..][0..beta_rows_usize], 0..) |gpu_value, i| {
            if (!std.math.isFinite(gpu_value)) {
                log.warn("M1 AMDGPU CS direct trusted SSM alpha/beta {s} row-range produced non-finite beta row {d}; beta remains host-computed and alpha may retry individually", .{ type_name, i });
        ...
        for (gpu_out[0..alpha_rows_usize], 0..) |gpu_value, i| {
            if (!std.math.isFinite(gpu_value)) {
                log.warn("M1 AMDGPU CS direct SSM alpha/beta {s} row-range produced non-finite alpha row {d}; alpha remains host-computed and beta may retry individually", .{ type_name, i });
                markDirectSsmQ8_0RowRangeFailed(state, layer, .alpha);
                return false;
            }
        ...
        for (gpu_out[alpha_rows_usize..][0..beta_rows_usize], 0..) |gpu_value, i| {
            if (!std.math.isFinite(gpu_value)) {
                log.warn("M1 AMDGPU CS direct SSM alpha/beta {s} row-range produced non-finite beta row {d}; beta remains host-computed and alpha may retry individually", .{ type_name, i });
                markDirectSsmQ8_0RowRangeFailed(state, layer, .beta);
                return false;
            }
        ...
        if (max_abs_delta > tolerance) {
            ...
            log.warn("M1 AMDGPU CS direct SSM alpha/beta {s} row-range mismatch: layer={d} kind={s} rows={d} max_abs_delta={d:.6} row={d}; {s} remains host-computed and the other projection may retry individually", .{
                type_name,
                layer,
                kind_name,
                total_rows,
    ...
    state.direct_ssm_alpha_q8_row_range_slice_successes += 1;
    state.direct_ssm_beta_q8_row_range_slice_successes += 1;
    tracking.ops.* += 2;
    mergeDirectComputeKind(tracking.kind, .dmmv_row_range);
    tracking.consumed.* = true;
    tracking.real_model_slice.* = true;
    if (tracking.decode_model_slices) |slices| slices.* += 2;
    const op_name = directSsmAlphaBetaRowRangeOpName(tensor_type, used_parallel64, trusted_no_oracle);
    if (trusted_no_oracle) {
...
fn directSsmBetaQ8_0RowRangeRows(
    ...
    const cols: u32 = @intCast(state.norm.len);
    if (!directSsmSupportsRowRange(beta_info.type_, cols)) return 0;
...
fn canAttemptDirectSsmBetaQ8_0RowRange(
    ...
    if (rows == 0) return false;
    const row_bytes = directSsmRowBytes(beta_info.type_, @intCast(state.norm.len));
    if (row_bytes == 0) return false;
    const beta_raw = model.tensorData(beta_info);
...
fn consumeDirectSsmBetaQ8_0RowRange(
    ...
    if (rows == 0) return false;
    const cols: u32 = @intCast(state.norm.len);
    const tensor_type = beta_info.type_;
    const type_name = directSsmRowRangeTypeName(tensor_type);
    ...
    if (!trusted_no_oracle) {
        const cpu_out = state.row_scratch[rows_usize..][0..rows_usize];
        matvecRawDirectSerial(beta_raw, tensor_type, state.norm, null, 0, rows, cpu_out) catch {
            return false;
        };
    }
    directSsmDispatchRowRange(
        ...
        log.warn("M1 AMDGPU CS direct SSM beta {s} row-range unavailable ({s}); beta remains host-computed", .{ type_name, @errorName(err) });
        return false;
    };
    ...
    if (trusted_no_oracle) {
        for (gpu_out, 0..) |gpu_value, i| {
            if (!std.math.isFinite(gpu_value)) {
                log.warn("M1 AMDGPU CS direct trusted SSM beta {s} row-range produced non-finite row {d}; beta remains host-computed", .{ type_name, i });
                markDirectSsmQ8_0RowRangeFailed(state, layer, .beta);
                return false;
            }
        }
        ...
        for (gpu_out, 0..) |gpu_value, i| {
            if (!std.math.isFinite(gpu_value)) {
                log.warn("M1 AMDGPU CS direct SSM beta {s} row-range produced non-finite row {d}; beta remains host-computed", .{ type_name, i });
                markDirectSsmQ8_0RowRangeFailed(state, layer, .beta);
                return false;
            }
            const delta = @abs(gpu_value - cpu_out[i]);
        ...
        if (max_abs_delta > direct_ssm_beta_q8_0_row_range_tolerance) {
            log.warn("M1 AMDGPU CS direct SSM beta {s} row-range mismatch: layer={d} rows={d} max_abs_delta={d:.6} row={d}; beta remains host-computed", .{
                type_name,
                layer,
                rows,
                max_abs_delta,
    ...
    if (tracking.decode_model_slices) |slices| slices.* += 1;
    if (trusted_no_oracle) {
        log.info("M1 AMDGPU CS direct model slice consumed: direct_compute_ops={d} direct_compute_kind=dmmv_row_range op=ssm_beta_{s}_row_range_trusted phase=decode layer={d} rows={d} cols={d} trust_after_successes={d} validation=finite_only", .{
            tracking.ops.*,
            type_name,
            layer,
            rows,
        ...
        log.info("M1 AMDGPU CS direct model slice consumed: direct_compute_ops={d} direct_compute_kind=dmmv_row_range op=ssm_beta_{s}_row_range phase=decode layer={d} rows={d} cols={d} max_abs_delta={d:.6} max_row={d}", .{
            tracking.ops.*,
            type_name,
            layer,
            rows,
...
fn dispatchDirectRouterRowRange(
    ...
    row_bytes: usize,
    output: []f32,
) !DirectRouterDispatchResult {
    switch (tensor_type) {
        .f32 => {
            try boundary.dmmvF32RowRange(input, weights, rows, cols, output);
            return .{};
        },
        .q8_0 => {
...
fn canConsumeDirectSharedMoeGateUpQ4_0Slot(state: *const ScalarDecodeState) bool {
    return !state.direct_shared_moe_gate_up_row_range_done;
}

fn canConsumeDirectMoeGateUpQ4_0Param(state: *const ScalarDecodeState, param: *const MoeExpertWorker) bool {
    return if (param.is_shared)
        canConsumeDirectSharedMoeGateUpQ4_0Slot(state)
    else
...
fn consumeDirectMoeGateUpQ4_0TwoRoutedSlotsBeforeCpuTail(
    ...
    const row_bytes = rowBytesForType(.q4_0, cols);
    const range_bytes = range_len * row_bytes;
    const scratch = directMoeGateUpScratch(state, input_sum32, range_len * 8) orelse return false;
    const gpu_rows = scratch[0 .. range_len * 4];
    tracking.boundary.dmmvQ4_0FourRowRangesParallel64(
        first.ffn_norm,
        first.gate_raw[0..range_bytes],
        first.up_raw[0..range_bytes],
        ...
        log.warn("M1 AMDGPU CS direct MoE gate/up Q4_0 two-slot prefix-before-tail unavailable ({s}); gate/up projections remain host-computed", .{@errorName(err)});
        return false;
    };
    ...
    if (!trusted_no_oracle) {
        const cpu_rows = scratch[range_len * 4 .. range_len * 8];
        matvecRawDirectSerial(first.gate_raw, .q4_0, first.ffn_norm, input_sum32, 0, range_rows, cpu_rows[0..range_len]) catch return false;
        matvecRawDirectSerial(first.up_raw, .q4_0, first.ffn_norm, input_sum32, 0, range_rows, cpu_rows[range_len..][0..range_len]) catch return false;
        matvecRawDirectSerial(second.gate_raw, .q4_0, second.ffn_norm, input_sum32, 0, range_rows, cpu_rows[range_len * 2 ..][0..range_len]) catch return false;
        matvecRawDirectSerial(second.up_raw, .q4_0, second.ffn_norm, input_sum32, 0, range_rows, cpu_rows[range_len * 3 ..][0..range_len]) catch return false;
        ...
        for (candidates, 0..) |candidate, ci| {
            ...
            for (0..range_len) |i| {
                if (!std.math.isFinite(gate_gpu[i]) or !std.math.isFinite(up_gpu[i])) {
                    log.warn("M1 AMDGPU CS direct MoE gate/up Q4_0 two-slot prefix-before-tail produced non-finite expert_slot={d} row={d}; gate/up projections remain host-computed", .{ candidate.expert_slot, i });
                    return false;
                }
                const gate_delta = @abs(gate_gpu[i] - gate_cpu[i]);
                const up_delta = @abs(up_gpu[i] - up_cpu[i]);
        ...
        if (max_gate_delta > direct_moe_gate_up_q4_0_tolerance or max_up_delta > direct_moe_gate_up_q4_0_tolerance) {
            log.warn("M1 AMDGPU CS direct MoE gate/up Q4_0 two-slot prefix-before-tail mismatch: gate_delta={d:.6} gate_slot={d} gate_row={d} up_delta={d:.6} up_slot={d} up_row={d}; gate/up projections remain host-computed", .{
                max_gate_delta,
                max_gate_slot,
                max_gate_row,
                max_up_delta,
        ...
        for (gpu_rows, 0..) |gpu, i| {
            if (!std.math.isFinite(gpu)) {
                log.warn("M1 AMDGPU CS direct MoE gate/up Q4_0 two-slot prefix-before-tail produced non-finite output {d}; gate/up projections remain host-computed", .{i});
                return false;
            }
        }
    }
    ...
    if (tracking.decode_model_slices) |slices| slices.* += 4;
    if (trusted_no_oracle) {
        log.info("M1 AMDGPU CS direct model slice consumed: direct_compute_ops={d} direct_compute_kind=dmmv_row_range op=moe_expert_gate_up_q4_0_two_slots_prefix_before_cpu_tail phase=decode expert_slot_a={d} expert_id_a={d} expert_slot_b={d} expert_id_b={d} rows_per_projection={d} cols={d} chunks=4 trust_after_successes={d} validation=finite_only consumed_gpu_model_value=1", .{
            tracking.ops.*,
            candidates[0].expert_slot,
            first.expert_id,
            candidates[1].expert_slot,
        ...
        log.info("M1 AMDGPU CS direct model slice consumed: direct_compute_ops={d} direct_compute_kind=dmmv_row_range op=moe_expert_gate_up_q4_0_two_slots_prefix_before_cpu_tail phase=decode expert_slot_a={d} expert_id_a={d} expert_slot_b={d} expert_id_b={d} rows_per_projection={d} cols={d} chunks=4 max_gate_delta={d:.6} max_gate_slot={d} max_gate_row={d} max_up_delta={d:.6} max_up_slot={d} max_up_row={d} consumed_gpu_model_value=1", .{
            tracking.ops.*,
            candidates[0].expert_slot,
            first.expert_id,
            candidates[1].expert_slot,
...
fn consumeDirectMoeGateUpQ4_0PrefixBeforeCpuTail(
    state: *ScalarDecodeState,
    maybe_tracking: ?DirectComputeTracking,
    param: *MoeExpertWorker,
    expert_slot: usize,
    ...
    const tracking = maybe_tracking orelse return 0;
    if (tracking.phase != .decode) return 0;
    if (expert_slot >= consumed_prefix_rows.len or consumed_prefix_rows[expert_slot] != 0) return 0;
    if (!canConsumeDirectMoeGateUpQ4_0Param(state, param)) return 0;
    if (param.gate_type != .q4_0 or param.up_type != .q4_0) return 0;
    if (param.intermediate_dim < direct_moe_gate_up_q4_0_range_rows) return 0;
    ...
    const range_len: usize = @intCast(range_rows);
    const range_bytes = range_len * row_bytes;
    if (param.gate.len < range_len or param.up.len < range_len) return 0;
    if (param.gate_raw.len < range_bytes or param.up_raw.len < range_bytes) return 0;

    const scratch = directMoeGateUpScratch(state, input_sum32, range_len * 4) orelse return 0;
    const gpu_rows = scratch[0 .. range_len * 2];
    tracking.boundary.dmmvQ4_0TwoRowRangesParallel64(
        param.ffn_norm,
        ...
        log.warn("M1 AMDGPU CS direct MoE gate/up Q4_0 prefix-before-tail unavailable ({s}); gate/up projections remain host-computed", .{@errorName(err)});
        return 0;
    };
    ...
    var max_up_row: u32 = 0;
    if (!trusted_no_oracle) {
        const cpu_rows = scratch[range_len * 2 .. range_len * 4];
        matvecRawDirectSerial(param.gate_raw, .q4_0, param.ffn_norm, input_sum32, 0, range_rows, cpu_rows[0..range_len]) catch {
            log.warn("M1 AMDGPU CS direct MoE gate/up Q4_0 prefix-before-tail CPU oracle failed; gate/up projections remain host-computed", .{});
            return 0;
        };
        matvecRawDirectSerial(param.up_raw, .q4_0, param.ffn_norm, input_sum32, 0, range_rows, cpu_rows[range_len..][0..range_len]) catch {
            log.warn("M1 AMDGPU CS direct MoE gate/up Q4_0 prefix-before-tail CPU oracle failed; gate/up projections remain host-computed", .{});
            return 0;
        };
        for (0..range_len) |i| {
            ...
            if (!std.math.isFinite(gate_gpu) or !std.math.isFinite(up_gpu)) {
                log.warn("M1 AMDGPU CS direct MoE gate/up Q4_0 prefix-before-tail produced non-finite row {d}; gate/up projections remain host-computed", .{i});
                return 0;
            }
            const gate_delta = @abs(gate_gpu - cpu_rows[i]);
            const up_delta = @abs(up_gpu - cpu_rows[range_len + i]);
            if (gate_delta > max_gate_delta) {
                max_gate_delta = gate_delta;
                max_gate_row = @intCast(i);
            }
        ...
        if (max_gate_delta > direct_moe_gate_up_q4_0_tolerance or max_up_delta > direct_moe_gate_up_q4_0_tolerance) {
            log.warn("M1 AMDGPU CS direct MoE gate/up Q4_0 prefix-before-tail mismatch: expert_slot={d} expert_id={d} shared={d} gate_delta={d:.6} gate_row={d} up_delta={d:.6} up_row={d}; gate/up projections remain host-computed", .{
                expert_slot,
                param.expert_id,
                @as(u32, @intFromBool(param.is_shared)),
                max_gate_delta,
                max_gate_row,
                max_up_delta,
                max_up_row,
        ...
        for (gpu_rows, 0..) |gpu, i| {
            if (!std.math.isFinite(gpu)) {
                log.warn("M1 AMDGPU CS direct MoE gate/up Q4_0 prefix-before-tail produced non-finite output {d}; gate/up projections remain host-computed", .{i});
                return 0;
            }
        }
    }
    ...
    consumed_prefix_rows[expert_slot] = range_rows;
    if (param.is_shared) {
        state.direct_shared_moe_gate_up_row_range_done = true;
    } else {
        state.direct_moe_gate_up_row_range_count += 1;
    ...
    tracking.real_model_slice.* = true;
    if (tracking.decode_model_slices) |slices| slices.* += 2;
    const op_name: []const u8 = if (param.is_shared)
        "moe_shared_gate_up_q4_0_prefix_before_cpu_tail"
    else
        "moe_expert_gate_up_q4_0_prefix_before_cpu_tail";
    if (trusted_no_oracle) {
        log.info("M1 AMDGPU CS direct model slice consumed: direct_compute_ops={d} direct_compute_kind=dmmv_row_range op={s} phase=decode expert_slot={d} expert_id={d} rows_per_projection={d} cols={d} chunks=2 trust_after_successes={d} validation=finite_only consumed_gpu_model_value=1", .{
            tracking.ops.*,
            op_name,
            expert_slot,
            param.expert_id,
        ...
        log.info("M1 AMDGPU CS direct model slice consumed: direct_compute_ops={d} direct_compute_kind=dmmv_row_range op={s} phase=decode expert_slot={d} expert_id={d} rows_per_projection={d} cols={d} chunks=2 max_gate_delta={d:.6} max_gate_row={d} max_up_delta={d:.6} max_up_row={d} consumed_gpu_model_value=1", .{
            tracking.ops.*,
            op_name,
            expert_slot,
            param.expert_id,
...
fn consumeDirectMoeGateUpQ4_0Rows(
    ...
    const tracking = maybe_tracking orelse return;
    if (tracking.phase != .decode) return;
    if (!canConsumeDirectMoeGateUpQ4_0Param(state, param)) return;
        ...
        param.up_raw.len >= range_bytes)
    {
        ...
        tracking.boundary.dmmvQ4_0TwoRowRangesParallel64(
            ...
            log.warn("M1 AMDGPU CS direct MoE gate/up Q4_0 row ranges unavailable ({s}); retrying two-row slice", .{@errorName(err)});
        ...
        if (range_dispatched) {
            for (0..range_len) |i| {
                ...
                const up_gpu = gpu_rows[range_len + i];
                if (!std.math.isFinite(gate_gpu) or !std.math.isFinite(up_gpu)) {
                    log.warn("M1 AMDGPU CS direct MoE gate/up Q4_0 row ranges produced non-finite row {d}; retrying two-row slice", .{i});
                    range_ok = false;
                    break;
                }
        ...
        if (range_ok and (max_gate_delta > direct_moe_gate_up_q4_0_tolerance or max_up_delta > direct_moe_gate_up_q4_0_tolerance)) {
            log.warn("M1 AMDGPU CS direct MoE gate/up Q4_0 row ranges mismatch: expert_slot={d} expert_id={d} shared={d} gate_delta={d:.6} gate_row={d} up_delta={d:.6} up_row={d}; retrying two-row slice", .{
                expert_slot,
                param.expert_id,
                @as(u32, @intFromBool(param.is_shared)),
                max_gate_delta,
        ...
        if (range_ok) {
            ...
            if (tracking.decode_model_slices) |slices| slices.* += 2;
            const op_name: []const u8 = if (param.is_shared)
                "moe_shared_gate_up_q4_0_row_ranges_parallel64"
            else
                "moe_expert_gate_up_q4_0_row_ranges_parallel64";
            log.info("M1 AMDGPU CS direct model slice consumed: direct_compute_ops={d} direct_compute_kind=dmmv_row_range op={s} phase=decode expert_slot={d} expert_id={d} rows_per_projection={d} cols={d} chunks=2 max_gate_delta={d:.6} max_gate_row={d} max_up_delta={d:.6} max_up_row={d} consumed_gpu_model_value=1", .{
                tracking.ops.*,
                op_name,
                expert_slot,
    ...
    tracking.boundary.dmmvQ4_0TwoRows(
        ...
        log.warn("M1 AMDGPU CS direct MoE gate/up Q4_0 rows unavailable ({s}); activations remain host-computed", .{@errorName(err)});
        return;
    };
    ...
    if (!std.math.isFinite(gpu_gate) or !std.math.isFinite(gpu_up)) {
        log.warn("M1 AMDGPU CS direct MoE gate/up Q4_0 rows produced non-finite values; activations remain host-computed", .{});
        return;
    }
    ...
    if (gate_delta > direct_moe_gate_up_q4_0_tolerance or up_delta > direct_moe_gate_up_q4_0_tolerance) {
        log.warn("M1 AMDGPU CS direct MoE gate/up Q4_0 rows mismatch: expert_slot={d} expert_id={d} shared={d} gate_delta={d:.6} up_delta={d:.6}; activations remain host-computed", .{
            expert_slot,
            param.expert_id,
            @as(u32, @intFromBool(param.is_shared)),
            gate_delta,
    ...
    if (tracking.decode_model_slices) |slices| slices.* += 2;
    const op_name: []const u8 = if (param.is_shared)
        "moe_shared_gate_up_q4_0_rows"
    else
        "moe_expert_gate_up_q4_0_rows";
    log.info("M1 AMDGPU CS direct model slice consumed: direct_compute_ops={d} direct_compute_kind=dmmv_row_range op={s} phase=decode expert_slot={d} expert_id={d} rows=2 cols={d} gate_gpu={d:.6} up_gpu={d:.6} gate_abs_delta={d:.6} up_abs_delta={d:.6} consumed_gpu_model_value=1", .{
        tracking.ops.*,
        op_name,
        expert_slot,
        param.expert_id,
...
fn runMoeExpertsParallelPhased(state: *ScalarDecodeState, params: []MoeExpertWorker, direct_compute_tracking: ?DirectComputeTracking) bool {
    ...
    if (direct_compute_tracking != null) {
        ...
        for (params, 0..) |*param, i| {
            const prefix_rows = gate_up_prefix_rows[i];
            if (prefix_rows == 0 or prefix_rows >= param.intermediate_dim) continue;
            const row_bytes = rowBytesForType(.q4_0, @intCast(ffn_norm.len));
            const prefix_bytes = @as(usize, prefix_rows) * row_bytes;
            if (row_bytes == 0 or param.gate_raw.len < prefix_bytes or param.up_raw.len < prefix_bytes) return false;
            gate_up[i * 2].raw = param.gate_raw[prefix_bytes..];
            gate_up[i * 2].rows = param.intermediate_dim - prefix_rows;
            gate_up[i * 2].out = param.gate[prefix_rows..];
            gate_up[i * 2 + 1].raw = param.up_raw[prefix_bytes..];
            gate_up[i * 2 + 1].rows = param.intermediate_dim - prefix_rows;
            gate_up[i * 2 + 1].out = param.up[prefix_rows..];
        }
    }
    ...
    var down_input_sums: [moe_expert_parallel_max_workers]?[]const f32 = .{null} ** moe_expert_parallel_max_workers;
    var down_prefix_rows = [_]u32{0} ** moe_expert_parallel_max_workers;
    for (params, 0..) |*param, i| {
        if (param.use_gelu) {
        ...
        down_input_sums[i] = down_input_sum32;
        down[i] = .{
            .raw = param.down_raw,
    ...
    if (direct_compute_tracking != null) {
        _ = consumeDirectMoeDownQ4_0PrefixesBeforeCpuTail(
            state,
            direct_compute_tracking,
            params,
            down_input_sums[0..params.len],
            down_prefix_rows[0..params.len],
        );
        for (params, 0..) |*param, i| {
            if (down_prefix_rows[i] != 0) continue;
            if (!canConsumeDirectMoeDownQ4_0Param(state, param)) continue;
            down_prefix_rows[i] = consumeDirectMoeDownQ4_0PrefixBeforeCpuTail(
                state,
                direct_compute_tracking,
                param,
                i,
    ...
    for (params, 0..) |*param, i| {
        const direct_prefix_rows = down_prefix_rows[i];
        if (direct_prefix_rows != 0 and direct_prefix_rows < down[i].rows) {
            const row_bytes = rowBytesForType(.q4_0, param.intermediate_dim);
            const prefix_bytes = @as(usize, direct_prefix_rows) * row_bytes;
            down[i].raw = param.down_raw[prefix_bytes..];
            down[i].rows -= direct_prefix_rows;
            down[i].out = param.down[direct_prefix_rows..];
        }
    }
...
fn canConsumeDirectMoeDownQ4_0Slot(state: *const ScalarDecodeState) bool {
...
fn consumeDirectMoeDownQ4_0PrefixesBeforeCpuTail(
    state: *ScalarDecodeState,
    maybe_tracking: ?DirectComputeTracking,
    params: []MoeExpertWorker,
    input_sums: []const ?[]const f32,
    consumed_prefix_rows: []u32,
) bool {
    const tracking = maybe_tracking orelse return false;
    if (tracking.phase != .decode) return false;
    if (input_sums.len < params.len or consumed_prefix_rows.len < params.len) return false;

    const range_rows = direct_moe_down_q4_0_range_rows;
    const range_chunks = directMoeDownQ4_0RangeChunks(range_rows);
    if (range_chunks == 0) return false;
    const range_len: usize = @intCast(range_rows);
    const routed_available = direct_moe_down_q4_0_max_expert_slots -| state.direct_moe_down_row_range_count;
    const reserve_oracle_rows: usize = if (state.direct_moe_down_row_range_count < direct_moe_down_q4_0_trust_after_successes) range_len else 0;

    var candidates: [moe_expert_parallel_max_workers]DirectMoeDownQ4_0BatchCandidate = undefined;
    var batches: [moe_expert_parallel_max_workers]zinc_rt.cs.DmmvQ4_0RowRangeBatch = undefined;
    var candidate_count: usize = 0;
    var routed_count: u32 = 0;
    ...
    for (params, 0..) |*param, i| {
        if (consumed_prefix_rows[i] != 0) continue;
        if (param.is_shared) {
            if (shared_count != 0 or state.direct_shared_moe_down_row_range_done) continue;
        } else {
            if (routed_count >= routed_available) continue;
        }
        if (param.down_type != .q4_0) continue;
        if (param.hidden_dim < range_rows) continue;
        if (param.intermediate_dim == 0 or param.swiglu_out.len < param.intermediate_dim) continue;
        if (param.down.len < range_len) continue;

        const cols = param.intermediate_dim;
        if (cols == 0 or cols % 32 != 0) continue;
        const cols_usize: usize = @intCast(cols);
        const row_bytes = rowBytesForType(.q4_0, cols);
        const range_bytes = range_len * row_bytes;
        if (row_bytes == 0 or param.down_raw.len < range_bytes) continue;
        if (total_rows + range_len + reserve_oracle_rows > state.row_scratch.len) break;

        candidates[candidate_count] = .{
            .param = param,
            .expert_slot = i,
            .input_sum32 = input_sums[i],
            .output_offset = total_rows,
        };
        batches[candidate_count] = .{
            .input = param.swiglu_out[0..cols_usize],
            .weights_q4_0 = param.down_raw[0..range_bytes],
            .rows = range_rows,
            .cols = cols,
        };
        candidate_count += 1;
        total_rows += range_len;
        if (param.is_shared) {
            shared_count += 1;
        } else {
            routed_count += 1;
    ...
    if (candidate_count < 2) return false;

    const gpu_rows = state.row_scratch[0..total_rows];
    tracking.boundary.dmmvQ4_0RowRangeParallelBatches(
        batches[0..candidate_count],
        gpu_rows,
    ) catch |err| {
        log.warn("M1 AMDGPU CS direct MoE down Q4_0 batched prefixes unavailable ({s}); retrying per-expert prefixes", .{@errorName(err)});
        return false;
    };
    ...
    for (candidates[0..candidate_count]) |candidate| {
        const param = candidate.param;
        const rows = gpu_rows[candidate.output_offset..][0..range_len];
        var finite = true;
        for (rows, 0..) |gpu, row| {
            if (!std.math.isFinite(gpu)) {
                finite = false;
                if (param.is_shared) {
                    log.warn("M1 AMDGPU CS direct shared MoE down Q4_0 batched prefix produced non-finite row {d}; shared down projection remains host-computed", .{row});
                } else {
                    log.warn("M1 AMDGPU CS direct MoE expert down Q4_0 batched prefix produced non-finite row {d}; down projection remains host-computed", .{row});
                }
                break;
            }
        }
        ...
        const trusted_no_oracle = state.direct_moe_down_row_range_count >= direct_moe_down_q4_0_trust_after_successes;
        var max_down_delta: f32 = 0.0;
        var max_down_row: u32 = 0;
        if (!trusted_no_oracle) {
            if (total_rows + range_len > state.row_scratch.len) continue;
            const cpu_rows = state.row_scratch[total_rows .. total_rows + range_len];
            matvecRawDirectSerial(
                param.down_raw,
                .q4_0,
                param.swiglu_out[0..@as(usize, @intCast(param.intermediate_dim))],
                candidate.input_sum32,
                0,
                range_rows,
                cpu_rows,
            ) catch {
                if (param.is_shared) {
                    log.warn("M1 AMDGPU CS direct shared MoE down Q4_0 batched prefix CPU oracle failed; shared down projection remains host-computed", .{});
                } else {
                    log.warn("M1 AMDGPU CS direct MoE expert down Q4_0 batched prefix CPU oracle failed; down projection remains host-computed", .{});
                }
                continue;
            };
            for (0..range_len) |i| {
                const delta = @abs(rows[i] - cpu_rows[i]);
                if (delta > max_down_delta) {
                    max_down_delta = delta;
                    max_down_row = @intCast(i);
                }
            }
            if (max_down_delta > direct_moe_down_q4_0_tolerance) {
                if (param.is_shared) {
                    log.warn("M1 AMDGPU CS direct shared MoE down Q4_0 batched prefix mismatch: expert_slot={d} down_delta={d:.6} down_row={d}; shared down projection remains host-computed", .{
                        candidate.expert_slot,
                        max_down_delta,
                        max_down_row,
                    });
                } else {
                    log.warn("M1 AMDGPU CS direct MoE expert down Q4_0 batched prefix mismatch: expert_slot={d} expert_id={d} down_delta={d:.6} down_row={d}; down projection remains host-computed", .{
                        candidate.expert_slot,
                        param.expert_id,
                        max_down_delta,
                        max_down_row,
        ...
        @memcpy(param.down[0..range_len], rows);
        consumed_prefix_rows[candidate.expert_slot] = range_rows;
        if (param.is_shared) {
        ...
        if (param.is_shared) {
            if (trusted_no_oracle) {
                log.info("M1 AMDGPU CS direct model slice consumed: direct_compute_ops={d} direct_compute_kind=dmmv_row_range op=moe_shared_down_q4_0_prefix_before_cpu_tail_batched phase=decode expert_slot={d} rows={d} cols={d} chunks={d} batch_ranges={d} trust_after_successes={d} validation=finite_only consumed_gpu_model_value=1", .{
                    tracking.ops.*,
                    candidate.expert_slot,
                    range_rows,
                ...
                log.info("M1 AMDGPU CS direct model slice consumed: direct_compute_ops={d} direct_compute_kind=dmmv_row_range op=moe_shared_down_q4_0_prefix_before_cpu_tail_batched phase=decode expert_slot={d} rows={d} cols={d} chunks={d} batch_ranges={d} max_down_delta={d:.6} max_down_row={d} consumed_gpu_model_value=1", .{
                    tracking.ops.*,
                    candidate.expert_slot,
                    range_rows,
                    param.intermediate_dim,
            ...
            if (trusted_no_oracle) {
                log.info("M1 AMDGPU CS direct model slice consumed: direct_compute_ops={d} direct_compute_kind=dmmv_row_range op=moe_expert_down_q4_0_prefix_before_cpu_tail_batched phase=decode expert_slot={d} expert_id={d} rows={d} cols={d} chunks={d} batch_ranges={d} trust_after_successes={d} validation=finite_only consumed_gpu_model_value=1", .{
                    tracking.ops.*,
                    candidate.expert_slot,
                    param.expert_id,
                    range_rows,
                ...
                log.info("M1 AMDGPU CS direct model slice consumed: direct_compute_ops={d} direct_compute_kind=dmmv_row_range op=moe_expert_down_q4_0_prefix_before_cpu_tail_batched phase=decode expert_slot={d} expert_id={d} rows={d} cols={d} chunks={d} batch_ranges={d} max_down_delta={d:.6} max_down_row={d} consumed_gpu_model_value=1", .{
                    tracking.ops.*,
                    candidate.expert_slot,
                    param.expert_id,
                    range_rows,
...
fn consumeDirectMoeDownQ4_0PrefixBeforeCpuTail(
    ...
    const tracking = maybe_tracking orelse return 0;
    if (tracking.phase != .decode) return 0;
    if (!canConsumeDirectMoeDownQ4_0Param(state, param)) return 0;
    if (param.down_type != .q4_0) return 0;
    if (param.hidden_dim < direct_moe_down_q4_0_range_rows) return 0;
    if (param.intermediate_dim == 0 or param.swiglu_out.len < param.intermediate_dim) return 0;
    ...
    tracking.boundary.dmmvQ4_0RowRangeParallelChunks(
        ...
        if (param.is_shared) {
            log.warn("M1 AMDGPU CS direct shared MoE down Q4_0 prefix-before-tail unavailable ({s}); shared down projection remains host-computed", .{@errorName(err)});
        } else {
            log.warn("M1 AMDGPU CS direct MoE expert down Q4_0 prefix-before-tail unavailable ({s}); down projection remains host-computed", .{@errorName(err)});
        }
        return 0;
    };

    for (0..range_len) |i| {
        if (!std.math.isFinite(gpu_rows[i])) {
            if (param.is_shared) {
                log.warn("M1 AMDGPU CS direct shared MoE down Q4_0 prefix-before-tail produced non-finite row {d}; shared down projection remains host-computed", .{i});
            } else {
                log.warn("M1 AMDGPU CS direct MoE expert down Q4_0 prefix-before-tail produced non-finite row {d}; down projection remains host-computed", .{i});
            }
            return 0;
        }
    }
    ...
    var max_down_row: u32 = 0;
    if (!trusted_no_oracle) {
        const cpu_rows = state.row_scratch[range_len .. range_len * 2];
        matvecRawDirectSerial(
            param.down_raw,
            .q4_0,
            param.swiglu_out[0..cols_usize],
            ...
            if (param.is_shared) {
                log.warn("M1 AMDGPU CS direct shared MoE down Q4_0 prefix-before-tail CPU oracle failed; shared down projection remains host-computed", .{});
            } else {
                log.warn("M1 AMDGPU CS direct MoE expert down Q4_0 prefix-before-tail CPU oracle failed; down projection remains host-computed", .{});
            }
            return 0;
        };
        for (0..range_len) |i| {
            const delta = @abs(gpu_rows[i] - cpu_rows[i]);
            if (delta > max_down_delta) {
                max_down_delta = delta;
                max_down_row = @intCast(i);
            }
        ...
        if (max_down_delta > direct_moe_down_q4_0_tolerance) {
            if (param.is_shared) {
                log.warn("M1 AMDGPU CS direct shared MoE down Q4_0 prefix-before-tail mismatch: expert_slot={d} down_delta={d:.6} down_row={d}; shared down projection remains host-computed", .{
                    expert_slot,
                    max_down_delta,
                    max_down_row,
                });
            } else {
                log.warn("M1 AMDGPU CS direct MoE expert down Q4_0 prefix-before-tail mismatch: expert_slot={d} expert_id={d} down_delta={d:.6} down_row={d}; down projection remains host-computed", .{
                    expert_slot,
                    param.expert_id,
                    max_down_delta,
                    max_down_row,
...
test "direct decode model slice policy defaults to LM-head plus router proofs" {
    ...
    try std.testing.expect(!shouldTrackDirectLmHeadDecode(16, false, 0));
}
...
test "direct MoE down Q4_0 prefix uses chunk-aligned row ranges" {
    try std.testing.expectEqual(@as(u32, 64), direct_moe_gate_up_q4_0_range_rows);
...
test "direct SSM row-range budget, trust and per-slice reset are bounded" {
    try std.testing.expectEqual(@as(u32, direct_ssm_q8_0_row_range_max_successes_default), directSsmQ8_0RowRangeMaxSuccessesForEnv(null));
    ...
    try std.testing.expectEqual(@as(u32, direct_ssm_q8_0_row_range_max_successes_default), directSsmQ8_0RowRangeMaxSuccessesForEnv("bad"));
    ...
    var state: ScalarDecodeState = undefined;
    state.direct_ssm_alpha_q8_row_range_done_mask = mask;
    state.direct_ssm_beta_q8_row_range_done_mask = mask;
    state.direct_ssm_alpha_q8_row_range_failed_mask = directSsmLayerBit(3);
    state.direct_ssm_beta_q8_row_range_failed_mask = directSsmLayerBit(5);
    state.direct_ssm_alpha_q8_row_range_successes = 7;
    state.direct_ssm_beta_q8_row_range_successes = 8;
    ...
    try std.testing.expect(!canConsumeDirectSharedMoeGateUpQ4_0Slot(&state));
    ...
    try std.testing.expect(!canTrustDirectSsmRowRangeWithoutOracle(&state, .q4_0));

    state.beginDirectDecodeSlice();
    try std.testing.expectEqual(@as(u128, 0), state.direct_ssm_alpha_q8_row_range_done_mask);
    try std.testing.expectEqual(@as(u128, 0), state.direct_ssm_beta_q8_row_range_done_mask);
    try std.testing.expectEqual(directSsmLayerBit(3), state.direct_ssm_alpha_q8_row_range_failed_mask);
    try std.testing.expectEqual(directSsmLayerBit(5), state.direct_ssm_beta_q8_row_range_failed_mask);
    try std.testing.expectEqual(@as(u32, 7), state.direct_ssm_alpha_q8_row_range_successes);
...
test "direct SSM row-range accepts F32 and Q8_0 projection rows" {
    ...
    var norm: [2048]f32 = undefined;
    var alpha: [32]f32 = undefined;
    var beta: [32]f32 = undefined;
    var scratch: [64]f32 = undefined;
    var state: ScalarDecodeState = undefined;
    state.norm = &norm;
    state.alpha = &alpha;
    state.beta = &beta;
    state.row_scratch = &scratch;

    try std.testing.expectEqual(@as(u32, 32), directSsmAlphaQ8_0RowRangeRows(cfg, &state, testTensorInfo(.f32)));
    try std.testing.expectEqual(@as(u32, 32), directSsmBetaQ8_0RowRangeRows(cfg, &state, testTensorInfo(.f32)));
    try std.testing.expectEqual(@as(u32, 32), directSsmAlphaQ8_0RowRangeRows(cfg, &state, testTensorInfo(.q8_0)));
    try std.testing.expectEqual(@as(u32, 0), directSsmAlphaQ8_0RowRangeRows(cfg, &state, testTensorInfo(.q4_0)));
}
